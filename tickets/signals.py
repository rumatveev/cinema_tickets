...
from .models import Order, Showing, Status
...
#         showing_obj.status = Status.SOLD_OUT.value
...
def mark_outdated_showings(sender, **kwargs):
    ...
    Showing.objects.filter(start__lte=threshold).update(status=Status.OUTDATED.value)